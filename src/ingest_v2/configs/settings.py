...
class SettingsV2:
    ...
    EMBED_MODEL_REVISION: str = os.getenv("EMBED_MODEL_REVISION", "")
    EMBED_DIM: int = int(os.getenv("EMBED_DIM", "3072"))
    EMBED_PROVIDER: str = os.getenv(
    ...
    PIPELINE_STORAGE_ROOT: str = os.getenv("PIPELINE_STORAGE_ROOT") or str(
        _DEFAULT_PIPELINE_STORAGE
    )
    ENTITIES_CACHE_DIR: str = os.getenv("ENTITIES_CACHE_DIR") or str(
        Path(PIPELINE_STORAGE_ROOT) / "entities_cache"
    )
    # Where to store enrichment sidecars (kept outside Parent docs)
    ROUTER_CACHE_DIR: str = os.getenv("ROUTER_CACHE_DIR") or str(
        Path(PIPELINE_STORAGE_ROOT) / "router_cache"
    )
    SPEAKER_EMBED_CACHE_DIR: str = os.getenv("SPEAKER_EMBED_CACHE_DIR") or str(
        Path(PIPELINE_STORAGE_ROOT) / "speaker_embeds"
    )
    SPEAKER_MAP_DIR: str = os.getenv("SPEAKER_MAP_DIR") or str(
        Path(PIPELINE_STORAGE_ROOT) / "speaker_maps"
    )
    VOICE_LIBRARY_DIR: str = os.getenv("VOICE_LIBRARY_DIR") or str(
        Path(PIPELINE_STORAGE_ROOT) / "voices"
    )

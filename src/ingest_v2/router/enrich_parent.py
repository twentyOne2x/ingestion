...
def _summarize_sentences_for_prompt(sentences: List[Dict[str, Any]], cap: int) -> List[Dict[str, Any]]:
    if not sentences:
        return []
...
def _build_messages(meta: Dict[str, Any], sentences: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    ...
    user = {
        ...
        "want_fields": [
            "description", "topic_summary", "router_tags", "aliases",
            "canonical_entities", "is_explainer", "router_boost"
        ],
        "format": {
            "type": "object",
            "properties": {
...
def enrich_parent_router_fields(meta: Dict[str, Any], sentences: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Lazy import to keep deps optional in test
